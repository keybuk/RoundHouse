...
extension Purchase {
    ...
    ///
    /// After calling this method, the model at the zero-indexed `origin` position with the set of `models`
    ...
    /// the model currently at the `destination` index; while when moving a model to a higher position, the model
    ...
    func moveModelAt(_ origin: Int, to destination: Int) {
